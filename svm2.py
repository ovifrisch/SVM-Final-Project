...
class SVM:
	...
	def __takeStep(self, i1, i2, E2):
		...
		for i in range(self.__size):
			if (self.__alphas[i] > 0 and self.__alphas[i] < self.__C):
				self.__error_cache[i] += t1 * self.__kernel.apply(self.__xs[i1, :], self.__xs[i, :]) + t2 * self.__kernel.apply(self.__xs[i2, :], self.__xs[i, :]) - delta_b
	...
	def __u(self, x=None, idx=None):
		...
		zipped = zip(kernel_vector, self.__ys, self.__alphas)
		products = [np.prod(tup) for tup in zipped]

		return sum(products) - self.__b

		# return np.sum(np.multiply(kernel_vec, self.__ys, self.__alphas)) - self.__b
